...
@section('content')
    <div class="page-container">
        ...
        <div class="header-info">
            <h1><i class="fas fa-users-cog me-2"></i>Gestión de Comprobantes</h1>
        </div>
                    ...
                    <tbody>
                        @forelse($empleados as $empleado)
                            <tr>
                                ...
                                <td><a href="mailto:{{ $empleado->email }}" class="link-primary">{{ $empleado->email }}</a>
                                </td>
                                <td><a href="tel:{{ $empleado->telefono }}"
                                        class="link-primary">{{ $empleado->telefono }}</a></td>
                                <td>
                                ...
                                <td class="actions">
                                    <a href="{{ route('empleados.edit', $empleado->empleado_id) }}" class="btn-action edit"
                                        title="Editar" data-bs-toggle="tooltip">
                                        <i class="fas fa-pen"></i>
                                    </a>
                                    <form action="{{ route('empleados.destroy', $empleado->empleado_id) }}" method="POST"
                                        onsubmit="return confirm('¿Confirmas que deseas eliminar este empleado?')">
                                        @csrf
                                        @method('DELETE')
