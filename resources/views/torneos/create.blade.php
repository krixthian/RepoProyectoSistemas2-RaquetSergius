...
<div class="container">
    ...
    <form action="{{ route('torneos.store') }}" method="POST">
        ...
        <div class="form-group mb-3">
            ...
            <select name="evento_id" id="evento_id" class="form-control" required>
                ...
                @foreach($eventos as $evento)
                    <option value="{{ $evento->evento_id }}" {{ old('evento_id') == $evento->evento_id ? 'selected' : '' }}>{{ $evento->nombre_del_evento ?? 'Evento ID: ' . $evento->evento_id }}</option>
                @endforeach
            </select>
        ...
        <div class="form-group mb-3">
            ...
            <input type="number" name="num_equipos" id="num_equipos" class="form-control" value="{{ old('num_equipos') }}" required min="2">
